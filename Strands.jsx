...
export default function Strands({
  ...
  useEffect(() => {
    ...
    let glassProgram = new Program(gl, {
      vertex: VERT,
      fragment: GLASS_FRAG,
      uniforms: {
    ...
    function resize() {
      if (!ctn) return;
      const width = Math.max(ctn.offsetWidth || window.innerWidth, 320);
      const height = Math.max(ctn.offsetHeight || window.innerHeight, 240);
  ...
  return <div ref={ctnDom} className={`hero-strands ${className}`} style={{ width: '100%', height: '100%', minHeight: '100vh', ...style }} />;
}
